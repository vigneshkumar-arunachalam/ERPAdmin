    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response.status != '') {
        this.addUserForm1.patchValue({
          ...
          'pettyCashColorCode': response.user_edit_details[0].petty_color_code,
          ...
          'permissionAs': response.user_edit_details[0].perm_userId,
          ...
          'HRGroup': response.user_edit_details[0].hr_group_id,
          'oneInputControl': response.user_edit_details[0].role,
          
       
        });
        
        const formArray = new FormArray([]);
        for (let index = 0; index < response.user_edit_details.length; index++) {
        
          formArray.push(this.fb.group({
            "MS_designation": response.user_edit_details[index].designation,
            "MS_FromDate": response.user_edit_details[index].dob,
            "MS_ToDate":response.user_edit_details[index].dob,
            "MS_salaryAmt": response.user_edit_details[index].monthly_salary,
            "MS_perDayAmt": response.user_edit_details[index].per_day_salary,
            "MS_employerAmt": response.user_edit_details[index].con1,
            "MS_employeeAmt": response.user_edit_details[index].con2,
            "MS_contribution1": response.user_edit_details[index].con1,
            "MS_contribution2": response.user_edit_details[index].con2,
